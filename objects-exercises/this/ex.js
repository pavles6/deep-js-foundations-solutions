...
const deepJS = {
  ...
  printRecords(recordIds) {
    const records = recordIds.map(this.getStudentFromId.bind(this));
  ...
  paidStudentsToEnroll() {
    const recordsToEnroll = this.studentRecords.filter(
      this.needToEnroll.bind(this)
    );

    const idsToEnroll = recordsToEnroll.map(this.getStudentId.bind(this));
  ...
  remindUnpaid(recordIds) {
    const unpaidIds = recordIds.filter(this.notYetPaid.bind(this));

    this.printRecords(unpaidIds);
  },

  notYetPaid(studentId) {
    const record = this.getStudentFromId(studentId);
    return !record.paid;
  },
};
